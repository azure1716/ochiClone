...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Don't hide navbar if we're near the top of the page
  ...
  return (
    <div className={`w-full px-15 py-6 flex justify-between fixed z-[300] transition-transform duration-300 ease-in-out ${
      ...
      <div className='items flex gap-10 text-lg'>
        {["Services", "Our work", "About us", "Insights", "Contact us"].map((item, index) => (
          <a key={index} className={`capitalize ${index === 4 && "ml-80"}`}>{item}</a>
        ))}
      </div>
    </div>
  );
...
// Demo content to test the navbar
const App = () => {
  return (
    <div>
      <Navbar />
